...
/******************************************************************************
* funciton : save stream to FIFOFILE
******************************************************************************/
HI_S32 SAMPLE_COMM_VENC_SaveStream_FIFOFILE(int fd, VENC_STREAM_S* pstStream)
{
    HI_S32 i;
    char END[] = "\r";
    // char panicTest[10] = "test panic";

    for (i = 0; i < pstStream->u32PackCount; i++)
    {
        // printf("%d \n", pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset);
        // fwrite(pstStream->pstPack[i].pu8Addr + pstStream->pstPack[i].u32Offset,
        //        pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset, 1, fd);
        // printf("%d \n", sizeof (pstStream->pstPack[i].pu8Addr + pstStream->pstPack[i].u32Offset));
        
        write(fd, pstStream->pstPack[i].pu8Addr + pstStream->pstPack[i].u32Offset,
         pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset);
        // write(fd, (char*)&panicTest, sizeof(panicTest));
        write(fd, (char*)&END, sizeof(END));
        // fflush(fd);
        // close(fd);
    }
    // close(fd);
...
HI_VOID* SAMPLE_COMM_VENC_GetVencStreamProc(HI_VOID* p)
{
    ...
    int fd;
    // printf("Tao FIFO file \n");    
    mknod("MYFIFO", S_IFIFO|0640, 0);
    printf("Tao FIFO file \n");  
    // fd = open("MYFIFO", O_NONBLOCK|O_WRONLY);
    fd = open("MYFIFO", O_WRONLY);
    printf("Tao FIFO file sucess \n");
            ...
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    ...
                    if (i==1) {
                        SAMPLE_PRT("begin write file!\n");
                        SAMPLE_PRT("%d \n", stStream.u32PackCount);
                        time_t t = time(NULL);
                        struct tm *tm = localtime(&t);
                        SAMPLE_PRT("%s \n", asctime(tm));
                        SAMPLE_COMM_VENC_SaveStream_FIFOFILE(fd, &stStream);                        
                    }
#ifndef __HuaweiLite__
