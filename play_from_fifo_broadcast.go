	...
	for i := len(h264FilePackets) - 4; i >= 0; i-- {
		...
		if h264FilePackets[i] == 0 {
			...
			if res2 == 0 {
				...
				if res == 0 {
					lastHeaderLocation = i - 1
				} else {
					lastHeaderLocation = i
				}
				break
			}
		}
...
func main() {
	...
	go func() {
		...
		for {
			...
			for {
				...
				if h264Err == io.EOF {
					fmt.Print("num connect: %d", numConneting)
	...
	// Wait for peer connection to broadcast
	<-iceConnectedCtx.Done()
	...
	for {
		...
		peerConnection.OnICEConnectionStateChange(func(connectionState webrtc.ICEConnectionState) {
			fmt.Printf("Connection State has changed %s \n", connectionState.String())
			if connectionState.String() == "connected" {numConneting++}
			if connectionState.String() == "failed" {numConneting--}
			if connectionState == webrtc.ICEConnectionStateConnected {
				iceConnectedCtxCancel()				
			}
		})
